# EquiTensor: Core + Fairness (adversary + disentanglement module)
...
LAMDA = 0.1
SENSITIVE_DIM = 1 # number of sensitive attributes
TOTAL_LEN = 45960
...
def get_scopes_to_restore(rawdata_1d_dict, rawdata_2d_dict, rawdata_3d_dict):
    ...
    prefix_list = ['1d_data_process_', '2d_data_process_', '3d_data_process_',
            'fusion_layer_','3d_data_reconstruct_', '2d_data_reconstruct_',
            '1d_data_reconstruct_','branching_']
...
class Autoencoder:
    ...
    '''
    Core integrative model + adversary + disentanglement module
    '''
    def train_autoencoder(self, rawdata_1d_dict, rawdata_2d_dict, rawdata_3d_dict,
        ...
        # disentanglement module: feed the sensitive info to 2D and 3D reconstruction
        ...
        # final loss for the autoencoder: L = L(reconstruction) + (1- L(adv)) * lamda
        cost = total_loss +  G_loss
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(start_epoch, epochs):
                ...
                for itr in range(iterations):
                    ...
                    # dynamic lambda
                    # Ganin, Yaroslav, and Victor Lempitsky. "Unsupervised domain adaptation by backpropagation."
                    ...
                    ##################  train AE ##############################
                    batch_cost, batch_G_loss, batch_loss, batch_loss_dict, batch_rmse_dict, _ = sess.run([cost, G_loss,
                                        total_loss, loss_dict, rmse_dict, AE_optimizer], feed_dict=feed_dict_all)
                ...
                save_path = saver.save(sess, save_folder_path +'core_fairness_' +str(epoch)+'.ckpt', global_step=self.global_step)
                ...
                for itr in range(itrs):
                    ...
                    test_batch_cost, test_batch_G_loss, test_batch_loss,test_batch_loss_dict, test_batch_rmse_dict, _ = sess.run([cost, G_loss,
                            total_loss, loss_dict, rmse_dict, AE_optimizer], feed_dict= test_feed_dict_all)
...
class Autoencoder_entry:
    def __init__(self, train_obj,
        ...
        self.dim = dim
        self.lamda = lamda  # lambda for controlling fairness
