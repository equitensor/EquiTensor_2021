...
# 1) update AE, supply sensitive info map (binarized) as y, into decoder.
...
def main():
    ...
    # last dimension = ['age65', 'white_pop', 'edu_uni', 'income_high']
    ...
    if suffix == '':
        save_path =  './core_fairness_'+ 'dim'+ str(dim)  + '_lamda_'+ str(lamda) +'_' +  str(attribute)+'/'
    else:
        if key == '':
            save_path = './core_fairness_'+ 'dim' + str(dim) + '_lamda_'+ str(lamda)+'_' +  str(attribute)+ '_'+ suffix +'/'
        else:
            save_path = './core_fairness_'+ 'dim' + str(dim) + '_lamda_'+ str(lamda) +'_' +  str(attribute)+ '_'+ suffix+ '_' + key  +'/'
    ...
    if resume_training == False:
        if inference == False:
            print('Train Model')
            latent_representation = equitensor.Autoencoder_entry(train_obj,
    ...
    txt_name = save_path + 'core_fairness_' + 'dim_' + str(dim) +'_'  + timer + '.txt'
