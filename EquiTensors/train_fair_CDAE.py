...
def parse_args():
    ...
    parser.add_argument('lamda', nargs='?', type = float, help = 'lambda for fairness', default = 1)
...
################  read corrputed data ########################
    ...
    if suffix == '':
        save_path =  './fair_CDAE_'+ 'dim'+ str(dim) + '_lamda_' + str(lamda) + '/'
    else:
        if key == '':
            save_path = './fair_CDAE_'+ 'dim' + str(dim) +'_lamda_' + str(lamda) +'_'+ suffix  +'/'
        else:
            save_path = './fair_CDAE_'+ 'dim' + str(dim) + '_lamda_' + str(lamda) +'_'+ suffix+ '_' + key  +'/'
    ...
    if resume_training == False:
        if inference == False:
            print('Train Model')
            latent_representation = fair_CDAE.Autoencoder_entry(train_obj,
    ...
    txt_name = save_path + 'fair_CDAE_' + 'dim_' + str(dim) +'_'  + timer + '.txt'
